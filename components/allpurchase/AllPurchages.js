...
const AddPurchase = () => {
    ...
    const fetchProductsAndCategories = async (supplierId) => {
      try {
        const response = await fetch(
          `${process.env.API}/user/supplierid/${supplierId}`
        );
  ...
  return (
    <Box sx={{ p: 2 }}>
              ...
              <TableRow key={row.id}>
                ...
                <TableCell>
                  <TextField
                    ...
                    value={
                      product &&
                      product.find((prod) => prod?._id === row?.productName)
                        ?.productName
                    }
                  />
                </TableCell>
      ...
      <Modal open={open} onClose={handleClose} sx={modalBackdropStyle}>
        <Box sx={modalStyle}>
            ...
            <Grid item xs={12} sm={6}>
              <FormControl fullWidth variant="outlined">
                ...
                >
                  {categories.map((c) => (
                    <MenuItem
                      key={c?.categoryNameId?._id}
                      value={c?.categoryNameId?._id}
                    >
